...
ImuOdom::ImuOdom(ros::NodeHandle& nh):nhi(nh) {
  //参数初始化
  imusub = nhi.subscribe("/imu/data_raw", 32, &ImuOdom::ImuCallback, this);
...
static Eigen::Matrix<double, 3, 3> ypr2R(const Eigen::Matrix<double, 3, 1> &ypr)
    {
...
void ImuOdom::calcOrientation(const geometry_msgs::Vector3 &msg) {
  ...
  // Eigen::Matrix<double, 3, 1> sss;
  // sss << msg.z * deltaT, msg.y * deltaT, msg.x * deltaT;
  // // std::cout << ypr2R(sss) << std::endl;
  // auto rotation = ypr2R(sss);
  
  double sigma =
      std::sqrt(std::pow(msg.x, 2) + std::pow(msg.y, 2) + std::pow(msg.z, 2)) *
      deltaT;
  // auto rotation = (Eigen::Matrix3d::Identity() + (std::sin(sigma) / sigma) * B -
  //               ((1 - std::cos(sigma)) / std::pow(sigma, 2)) * B * B);
  auto rotation = (Eigen::Matrix3d::Identity() + (std::sin(sigma) / sigma) * B -
                ((1 - std::cos(sigma)) / std::pow(sigma, 2)) * B * B);
      
  point.orien =  point.orien * rotation;
